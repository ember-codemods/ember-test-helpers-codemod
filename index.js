function transform(file, api) {
  ...
  root
    ...
    .filter(({ node }) => isJQueryTextExpression(j, node))
    .replaceWith(({ node }) => createTextExpression(j, node.callee.object.arguments))
...
function isJQueryTextExpression(j, node) {
  return j.CallExpression.check(node)
    && j.MemberExpression.check(node.callee)
...
function createTextExpression(j, findArgs) {
