...
function transform(file, api) {
  ...
  let triggerReplacements = replacements
    ...
    .insertAfter((path) => createTriggerExpression(j, path.node.expression.callee.object.argument.arguments[0], path.node.expression.arguments[0]))
  ...
  let changeReplacements = replacements
    ...
    .map((path) => path.parent.parent.parent)
    .insertAfter((path) => createTriggerExpression(j, path.node.expression.callee.object.argument.arguments[0], j.literal('change')))
    .replaceWith((path) => j.expressionStatement(path.node.expression.callee.object))
    .forEach((path) => makeParentFunctionAsync(j, path));
